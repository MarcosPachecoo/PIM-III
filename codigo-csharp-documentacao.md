# Codigo C# - Painel Administrativo Napoli Express

Este documento reune os codigos do painel administrativo desenvolvido em C# com ASP.NET Core. O arquivo principal e o Program.cs, responsavel pelas rotas, autenticacao, telas HTML do painel e conexao com o banco MySQL.
...
static Pedido InserirPedido(string conexaoBanco, PedidoEntrada pedidoEntrada)
{
    ...
    try
    {
        ...
        using var comandoPedido = new MySqlCommand(
            """
            INSERT INTO pedidos
            (endereco_entrega, forma_pagamento, total_pedido, status_pedido, data_hora, id_cliente)
            VALUES
            (@endereco, @pagamento, @total, @status, NOW(), @idCliente);
...
static void InserirItemPedido(MySqlConnection conexao, MySqlTransaction transacao, int idPedido, ItemPedido item)
{
    ...
    if (tipoItem == "Pizza")
    {
        ...
        using var comandoPizza = new MySqlCommand(
            """
            INSERT INTO pizzas
            (tamanho, quantidade_fatias, sabores, borda, valor_borda, id_item)
            VALUES
            (@tamanho, @fatias, @sabores, @borda, @valorBorda, @idItem);
            """,
            conexao,
            transacao);
        ...
        comandoPizza.Parameters.AddWithValue("@valorBorda", 0);
...
static DetalhesPizza ExtrairDetalhesPizza(string nome)
{
    ...
    {
        return new DetalhesPizza("", 0, nome.Replace("Pizza", "").Trim(), "Sem borda");
    }
    ...
    return new DetalhesPizza(tamanho, fatias, sabores, borda);
}
...
record DetalhesPizza(string Tamanho, int QuantidadeFatias, string Sabores, string Borda);
...
## PainelAdministrativo.csproj

O arquivo abaixo nao e C#, mas faz parte da configuracao do projeto .NET e mostra o uso do pacote MySqlConnector, necessario para conectar o painel ao banco MySQL.
